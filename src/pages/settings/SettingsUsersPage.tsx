...
export function SettingsUsersPage() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      setMessage(tmp ? `Usuario guardado. Contraseña temporal: ${tmp}` : 'Usuario guardado');
    },
  });
  ...
  const resetMutation = useMutation({
    ...
    onSuccess: (res) => {
      setMessage(`Contraseña temporal: ${res.data.temporaryPassword}`);
    },
  });
            ...
            {users.map((user) => (
              <tr key={user.id}>
                ...
                <td className="px-4 py-3 text-right">
                  <button
                    type="button"
                    className="mr-2 text-sm text-primary-700 hover:underline"
                    onClick={() => {
                      setEditing(user);
                      setCreating(false);
                    }}
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (confirm('¿Generar contraseña temporal?')) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      ...
      >
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <input className={field} placeholder="Nombre" value={firstName} onChange={(e) => setFirstName(e.target.value)} required />
          <input className={field} placeholder="Apellidos" value={lastName} onChange={(e) => setLastName(e.target.value)} required />
        </div>
        {!user && (
        ...
        <select className={field} value={roleId} onChange={(e) => setRoleId(e.target.value)} required>
          {roles.map((r) => (
